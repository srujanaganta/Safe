...
class DBHelper extends SQLiteOpenHelper {
    ...
    public DBHelper(Context context) {
        ...
        super(context, "rupermtrubnikovgpsDB", null, 4);
    ...
    public static void updateFavIcon(Context context, ImageButton btn) {
        ...
        try {
            ...
            Intent icon_intent = new Intent(Intent.ACTION_SEND);
        ...
        if (!resInfo.isEmpty()) {
            for (ResolveInfo info : resInfo) {
                ...
                if (info.activityInfo.name.toLowerCase().equalsIgnoreCase(localPrefs.getString("prefFavAct", ""))) { //|| info.activityInfo.name.toLowerCase().contains(pckg))
                    share.putExtra(Intent.EXTRA_SUBJECT, context.getString(R.string.share_topic));
                    share.putExtra(Intent.EXTRA_TEXT, crds);
